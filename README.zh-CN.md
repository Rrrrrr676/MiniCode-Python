...
除了 TUI 与 Headless，MiniCode 现在提供一个仅监听 loopback 的本地 Web 控制台。它支持会话恢复、流式回答、工具卡片、显式失败、权限审批和只读工作区 Diff。
...
然后打开 `http://127.0.0.1:8765`。服务默认且固定绑定 `127.0.0.1`，浏览器不会收到 API Key、Token 或完整环境变量。前端开发模式使用 `cd web && npm run dev`，并将 `/api` 代理到本地 Web 服务。
